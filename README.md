...
**New**: The dashboard now includes a **Metrics page** for analyzing event logs with charts and analytics. Switch between Observatory and Metrics using the tab navigation at the top of each page.
...
- **Tab navigation**: Switch between Observatory and Metrics at the top
...
- **💾 Save as Event Log**: Export the current conversation as a CSV event log for analysis

#### Metrics Page (/metrics)

- **Tab navigation**: Switch back to Observatory at the top
- **Event log selector**: Choose from any saved conversation logs in `generated_event_log/`
- **KPI metrics**: Total events, unique activities, handovers, token usage
- **Agent workload chart**: Bar chart showing events handled by each agent
- **Activity duration chart**: Average duration for each activity type

### Workflow

1. **Run a conversation** in the Observatory (select scenario, click "Run Conversation")
2. **Save the log** using the "💾 Save as Event Log" button (enabled when conversation completes)
3. **Switch to Metrics** by clicking the "Metrics" tab at the top
4. **Select any log** from the dropdown to analyze (defaults to most recent)
5. **View analytics** with charts and KPIs
6. **Switch back** to Observatory to run more simulations

You can also view metrics from logs generated by `poetry run simulate --export-logs` without running the dashboard conversation first.
...
**Event Log Saving**: Dashboard events are captured in real-time and can be exported to OCEL-compatible CSV format. Note that dashboard logs don't include token counts or precise durations (these fields will be null). For complete metrics with all fields, use `poetry run simulate --export-logs` which generates logs from MLflow traces.
